...
class sspmod_fedlab_BasicSPTest extends sspmod_fedlab_Tester {
	...
	function __construct($idpmetadata, $metadata, $metadataXML, $initurl, $initslo, $attributeurl) {
		parent::__construct($idpmetadata, $metadata, $metadataXML, $initurl, $initslo, $attributeurl);		
	...
	public function simpleTest() {
		...
		if (!isset($result['Request'])) {
			//echo '<pre>Request::'; print_r($result); exit;
		...
		$this->log('debug', ' Trying to access attributeurl: ' . $this->attributeurl);
		error_log(' Trying to access attributeurl: ' . $this->attributeurl);
	...
	public function expectAuthenticationRequest($result) {
		...
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'authnrequest', 'Authentication Request found', (SimpleSAML_Utilities::formatXMLString($requestRaw)));
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'relaystate', 'Authentication Request RelayState found', var_export($relaystate, TRUE));
	}


	public function expectLogoutResponse($response) {
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'authnrequest', 'Logout Response found', SimpleSAML_Utilities::formatXMLString($response));

	}
	

	public function logoutrequestPrepared($lr) {
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'authnrequest', 'Logout request prepared', (SimpleSAML_Utilities::formatXMLString($lr['Request'])));
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'relaystate', 'Logout request relaystate', var_export($lr['relaystate'], TRUE));
	}


	public function responsePrepared($samlResponse) {
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'authnresponse', 'Response sent back to provider', (SimpleSAML_Utilities::formatXMLString($samlResponse['Response'])));
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'responserelaystate', 'Response RelayState parameter', var_export($samlResponse['RelayState'], TRUE));
	...
	public function run($testrun) {
		...
		$this->setResultPage($result2['body']);
...
# 		if (strstr(htmlspecialchars_decode($body), '>andreas@uninett.no')) return FALSE;
#		if (preg_match('|[^;]andreas@uninett.no|', $body)) return TRUE;
		if (preg_match('/(?<!(?:&[Gg][Tt];)|(?:[Aa]ttribute[Vv]alue>))andreas@uninett.no/', $body)) return TRUE;
#		if (preg_match('|[^>]andreas@uninett.no|', $body)) return TRUE;
#		if (strstr($body, 'andreas@uninett.no')) return TRUE;
		return FALSE;
	}
	
	protected function setResultPage($res) {
		$sb = 'NA';
		if(preg_match('|<body.*?>(.*?)</body>|is', $res, $matches)) {
			$sb = strip_tags($matches[1], '<p><span><div><table><tr><td><ul><li><ol><dd><dt><dl><code><pre>');
		}
		$this->setResult(sspmod_fedlab_Tester::STATUS_INFO, null, 'resultpage', 'Resulting HTML page shown to user', ($sb));
	}
	...
	function getConfig($testrun) {
 	...
 	protected function expectedResult($testrun, $body, $debugoutput) {
		...
		if (strstr($body, 'andreas@uninett.no')) {
			$this->setResult(sspmod_fedlab_Tester::STATUS_OK, $testrun, $testrun, $this->testruns[$testrun]);
		} else {
			$this->setResult(sspmod_fedlab_Tester::STATUS_ERROR, $testrun, $testrun, $this->testruns[$testrun]);
		}
	...
	public function createResponseP($testrun, $request, $relayState = NULL) {
		return $this->createResponse($testrun, $request, $relayState = NULL);
	}
