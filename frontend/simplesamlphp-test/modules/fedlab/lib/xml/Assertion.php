 ...
 * @version $Id: Assertion.php 39 2010-07-28 16:28:44Z andreassolberg $
 */
class sspmod_fedlab_xml_Assertion implements SAML2_SignedElement {


	public $dateformat = 'Y-m-d\TH:i:s\Z';
	public $includeAuthn = TRUE;
		
	public $extracondition = NULL;
	public $addSubjectConfirmationData = TRUE;
	public $iterateSubjectConfirmationData = FALSE;
	public $subjectAddresses = array();
#	public $subjectConfirmationDestinations = array();
	...
	public $notOnOrAfterSubjectConfirmationData;
	...
	/**
	 * The protected key we should use to sign the assertion.
	 *
	 * The protected key can be NULL, in which case the assertion is sent unsigned.
	 *
	 ...
	 * @var boolean
	 */
	protected $requiredEncAttributes;
	 ...
	 */
	protected function parseSubject(DOMElement $xml) {
	 ...
	 * @param DOMElement $xml  The assertion XML element.
	 */
	protected function parseAuthnStatement(DOMElement $xml) {
	 ...
	 */
	protected function parseAttributes(DOMElement $xml) {
	 ...
	 * @param DOMElement $xml  The XML element with the assertion.
	 */
	protected function parseEncryptedAttributes(DOMElement $xml) {
	 ...
	 * @param DOMElement $xml  The assertion XML element.
	 */
	protected function parseSignature(DOMElement $xml) {
	...
	public function setNotOnOrAfter($notOnOrAfter) {
		...
		$this->notOnOrAfter = $notOnOrAfter;
		$this->notOnOrAfterSubjectConfirmationData = $notOnOrAfter;
	}
...
#		assert('is_string($destination) || is_null($destination)');
	...
	/**
	 * Retrieve the protected key we should use to sign the assertion.
	 *
	...
	/**
	 * Set the protected key we should use to sign the assertion.
	 *
	...
	/**
	 * Set the protected key we should use to encrypt the attributes.
	 *
	 * @param XMLSecurityKey|NULL $key
	 */
	...
	protected function addAttributeStatement(DOMElement $root) {
		...
		foreach ($this->attributes as $name => $values) {
			...
			foreach ($values as $value) {
				...
				} elseif ($value instanceof sspmod_fedlab_xml_Assertion) { 
					$valueXML = $value->toXML();
					$node = $document->importNode($valueXML, TRUE);
					$attributeValue->appendChild($node);
				} else {
