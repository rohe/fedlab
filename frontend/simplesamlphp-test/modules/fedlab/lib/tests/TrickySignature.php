...
class sspmod_fedlab_tests_TrickySignature extends sspmod_fedlab_BasicSPTest {


	
	protected function register() {
		$this->testruns = array(
			'trickysignature1' => 'SP SHOULD NOT accept an signed assertion embedded in an AttributeValue inside an unsigned assertion.',
			'trickysignature1b' => 'SP SHOULD NOT accept an signed assertion embedded in an AttributeValue inside an unsigned assertion. (Signature moved out...)',
		);
	}
...
#		print_r($requestId);
		...
		$innerAssertion = $this->getAssertion($testrun, $request, $fakeAttr, TRUE, TRUE);
		
		$realAttr['urn:inner:assertion'] = array($innerAssertion);
		
		if ($testrun === 'trickysignature1b') {
			$outerAssertion = $this->getAssertion($testrun, $request, $realAttr, TRUE, TRUE);
		} else {
			$outerAssertion = $this->getAssertion($testrun, $request, $realAttr, FALSE, TRUE);			
		}


		$response->setAssertions(array($outerAssertion));
	...
	protected function tweakResponseDOM($testrun, &$response) {
		...
		if ($testrun !== 'trickysignature1b') return;
		...
		$firstSignatureQ = $xpath->query("/saml2p:Response/saml2:Assertion/ds:Signature");
		$firstSignature = $firstSignatureQ->item(0);
		
		$secondSignatureQ = $xpath->query("//saml:Attribute/saml:AttributeValue/saml:Assertion/ds:Signature");
